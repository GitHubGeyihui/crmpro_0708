...
<title>需求分析管理</title>
    ...
    <script type="application/javascript">
        $(function(){
            $.ajax({
                type:'GET',
                url:'${pageContext.request.contextPath}/pro/showAllAnalysis',
                dataType:'json',
                success:function(rs){
                    console.info(rs);
                    $(rs).each(function(index,item){
                        // 将毫秒值转换成日期
                        var newDate = new Date(item.addtime);
                        var time1 = newDate.getFullYear()+"-"+(newDate.getMonth()+1)+"-"+newDate.getDate();

                        // 将毫秒值转换成日期
                        var newDate = new Date(item.updatetime);
                        ...
                        var tr ="<tr align='center' bgcolor=\"#FFFFFF\" onMouseMove=\"javascript:this.bgColor='#FCFDEE';\" onMouseOut=\"javascript:this.bgColor='#FFFFFF';\" height=\"22\" >\n" +
                            ...
                            "\t<td>"+time1+"</td>\n" +
                            "\t<td>"+time2+"</td>\n" +
                            "\t<td><a href=\"${pageContext.request.contextPath}/pro/downLoadFile?filename="+item.proname+"\">下载</a> | <a href=\"project-need-look.jsp\">查看详情</a></td>\n" +
                            "</tr>";
                        //添加到指定行后面
                        $("#ttr").after(tr);
...
<tr>
 <td height="26" background="skin/images/newlinebg3.gif">
  ...
  <td>
    <input type='button' class="coolbg np" onClick="location='project-need-add.jsp';" value='添加需求' />
 </td>
 </tr>
</table>
</td>
